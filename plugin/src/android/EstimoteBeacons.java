...
public class EstimoteBeacons extends CordovaPlugin
{
    ...
    {
        // FIXME this seems like a bad idea when taking threading in consideration 
        //       we're only using this for the bluetooth enable dialog and afterwards
        //       it's reset back to null to prevent these types of problems
    ...
    /** 
     * Check for Bluetooth low-power support and return result to bridge
     */
    public void hasBluetooth() {
        if (mBeaconManager.hasBluetooth()) {
            activeCallbackContext.success();
        } else {
            activeCallbackContext.error("No Bluetooth available");
        }
        // Be safe and unset the class property
        this.activeCallbackContext = null;

    }

    @Override
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        Log.i(LOGTAG, "activity-result");
        hasBluetooth();
    }
    ...
    private void startMonitoringForRegion(
            ...
            throws JSONException
    {
        Log.i(LOGTAG, "startMonitoringForRegion mall-connect");
